...
public:
    struct GizmoEventCallback : public osg::Drawable::EventCallback
    {
        virtual void event(osg::NodeVisitor *nv, osg::Drawable *drawable)
        {
            ...
            GizmoDrawable *gizmoDrawable =
                dynamic_cast<GizmoDrawable *>(drawable);
            ...
            for (osgGA::EventQueue::Events::const_iterator itr = events.begin();
                ...
                case osgGA::GUIEventAdapter::SCROLL:
                    // you would have other methods to select among gizmos, this
                    // is only an example
                    {
                        int mode = gizmoDrawable->getGizmoMode();
                        if (ea->getScrollingMotion()
                            == osgGA::GUIEventAdapter::SCROLL_UP)
                            mode = (mode == GizmoDrawable::NO_GIZMO
                                        ? GizmoDrawable::SCALE_GIZMO
                                        : mode - 1);
                        else if (ea->getScrollingMotion()
                                 == osgGA::GUIEventAdapter::SCROLL_DOWN)
                            mode = (mode == GizmoDrawable::SCALE_GIZMO
                                        ? GizmoDrawable::NO_GIZMO
                                        : mode + 1);
                ...
                case osgGA::GUIEventAdapter::RESIZE:
                    gizmoDrawable->setScreenSize(ea->getWindowWidth(),
                                                 ea->getWindowHeight());
                    break;
    ...
    {
        osg::Matrix matrix;
        for (int i = 0; i < 16; ++i) _editMatrix[i] = *(matrix.ptr() + i);
        _screenSize[0] = 800;
        _screenSize[1] = 600;
    ...
    GizmoDrawable(const GizmoDrawable &copy,
                  osg::CopyOp op = osg::CopyOp::SHALLOW_COPY)
        : osg::Drawable(copy, op), _transform(copy._transform),
          _gizmo(copy._gizmo), _mode(copy._mode)
    {
    }
    ...
    void setScreenSize(int w, int h)
    {
        _screenSize[0] = w;
        _screenSize[1] = h;
        if (_gizmo) _gizmo->SetScreenDimension(w, h);
    }
...
//     viewer.getCamera()->getGraphicsContext() ); if ( gw )
//     {
